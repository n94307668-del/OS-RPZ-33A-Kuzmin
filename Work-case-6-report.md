...
```bash
# Перегляд усіх створених користувачів
...
```bash
# Перемикання на користувача технічної підтримки
su - tech1

# Збір системної інформації
echo "=== Системна інформація ==="
uname -a                          # Версія ядра
cat /etc/os-release               # Дистрибутив ОС
hostname                          # Ім'я системи
...
id                                # ID користувача та групи
date                              # Системна дата та час

# Системні ресурси
echo "=== Системні ресурси ==="
free -h                           # Використання пам'яті
df -h                             # Використання диска
ps aux | head -10                 # Процеси
```
...
```zsh
# Перемикання на користувача-розробника
su - dev1

# Системна інформація з можливостями zsh
echo "=== Zsh оточення ==="
echo $SHELL                       # Підтвердження zsh
zsh --version                     # Версія Zsh

# Розширена навігація
echo "=== Операції з каталогами ==="
ls -la                            # Список файлів
cd /var/log && pwd                # Навігація та показ шляху
pushd /etc && dirs                # Стек каталогів
popd

# Розширений глоббінг (фіча zsh)
echo "=== Шаблони файлів ==="
ls /etc/*.conf(N)                 # Список .conf файлів
echo /usr/bin/z*                  # Файли, що починаються на 'z'
...
**Особливості Zsh:**
- **Таб-доповнення:** Натисніть `Tab` для автодоповнення
- **Підсвічування синтаксису:** Команди кольорові під час набору
- **Пошук по історії:** Натисніть `↑` для історії команд
...
```fish
# Перемикання на користувача-керівника
su - founder1

# Fish оточення
echo "=== Особливості Fish ==="
echo $SHELL                       # Підтвердження fish
fish --version                    # Версія Fish
...
# Демонстрація підсвічування
echo "Наберіть 'ls -la' та спостерігайте за кольором"

# Веб-налаштування (якщо доступно)
# fish_config                       # Відкриває налаштування в браузері

# Системна інформація
echo "=== Швидка статистика ==="
command uptime
command free -h
df -h /home                       # Використання home-розділу
```
...
**Перевірка:**
```bash
# Перевірка існування гостя, але без можливості входу
id guest1
getent passwd guest1
groups guest1

# Перегляд домашнього каталогу (створений, але недоступний)
ls -ld /home/guest1
...
## Зведена таблиця: Всі користувачі

| Користувач | Група | Оболонка | Доступ до shell | Домашній каталог |
|------------|-------|----------|-----------------|------------------|
| tech1 | techsupport | /bin/bash | ✅ Так | /home/tech1 |
| tech2 | techsupport | /bin/bash | ✅ Так | /home/tech2 |
| dev1 | developers | /usr/bin/zsh | ✅ Так | /home/dev1 |
| dev2 | developers | /usr/bin/zsh | ✅ Так | /home/dev2 |
| fin1 | financiers | /usr/sbin/nologin | ❌ Ні | /home/fin1 |
| fin2 | financiers | /usr/sbin/nologin | ❌ Ні | /home/fin2 |
| founder1 | founders | /usr/bin/fish | ✅ Так | /home/founder1 |
| founder2 | founders | /usr/bin/fish | ✅ Так | /home/founder2 |
| guest1 | guests | /usr/sbin/nologin | ❌ Ні | /home/guest1 |
| guest2 | guests | /usr/sbin/nologin | ❌ Ні | /home/guest2 |
...
```bash
# Список усіх користувачів в системі
cut -d: -f1 /etc/passwd
...
# Додавання користувача до додаткової групи
...
# Перевірка активності користувачів
who
w
last
```
...
*Звіт виконано в рамках самостійної роботи студента*  
*Work-case 6 - Керування користувачами Linux*
